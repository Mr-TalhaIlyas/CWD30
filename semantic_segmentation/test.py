...
from data_utils import collate, pallet
from models.model import SegNext
from models.ocr import ResNet_OCR
import models.smp as smp
from models.losses import FocalLoss, CrossEntropyLoss2d
...
g2c = lambda x : gray2color(x, use_pallet='cityscape', custom_pallet=pallet)
#%%
...
plt.axis('off')
#%%
print(f'Loading Encoder chkpt: {config["encoder_chkpt"]}')
if config['selected_model'] == 'segnext':
    model = SegNext(num_classes=config['num_classes'], in_channnels=3, embed_dims=[64, 128, 320, 512],
                    ffn_ratios=[4, 4, 4, 4], depths=[3, 5, 27, 3], num_stages=4,
                    dec_outChannels=1024, ls_init_val=float(config['layer_scaling_val']), 
                    drop_path=float(config['stochastic_drop_path']), config=config)

elif config['selected_model'] == 'unet':
    model = smp.Unet(encoder_name="resnet101", encoder_weights=config['encoder_chkpt'],     
                    in_channels=3, classes=config['num_classes'])

elif config['selected_model'] == 'deeplabv3plus':
    model = smp.DeepLabV3Plus(encoder_name="resnet101", encoder_weights=config['encoder_chkpt'],     
                                in_channels=3, classes=config['num_classes'])

elif config['selected_model'] == 'ocr':
    model = ResNet_OCR(config['num_classes'], feat_channels=[1024, 2048],
                       pretrained=config['encoder_chkpt'])

model = model.to('cuda' if torch.cuda.is_available() else 'cpu')

print(summary(model,
              input_size=(2, 3, config['img_height'], config['img_width']),
              depth=1))
...
# elif config['selected_model'] == 'deeplabv3plus':
#     model = smp.DeepLabV3Plus(encoder_name="resnet101", encoder_weights="imagenet",     
#                                 in_channels=3, classes=config['num_classes'])
